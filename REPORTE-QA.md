...
> - **Certificación final (build de producción, 41 rutas × 4 anchos):** consola **0 errores**; scroll horizontal **0**; axe `select-name`/`button-name`/`label` **0** (antes 42); contraste **874 → ~236 nodos** (residuo: acento de marca como texto pequeño — decisión de diseño pendiente); touch targets <34px a 360px **407 → 65** (residuo: chips/enlaces secundarios); `npm run build` OK (PWA precache 26 entradas); vitest 35/35 con consola limpia.
> - **Ola 4** (pulido objetivo): resueltos `nested-interactive` (tarjeta de Inicio) y `scrollable-region-focusable` (Estadísticas) → axe estructural en **0**; botones de icono pequeños (plan ✓, cronómetro ×, pager) subidos a 44px. Tras Ola 4, **todos los objetivos táctiles cumplen WCAG AA 2.5.8 (≥24×24px)** — los 43 que aún marca el crawler lo hacen solo contra su umbral aspiracional de 34px (breadcrumbs 27×36, switches 42×30, pager 25×44), no contra el estándar.
> - **Ola 4b/4c** (contraste, aprobado): tokens de "color como texto" separados del color de relleno. `--accent-text` (#0F6F8B claro / #1B8FBE oscuro) y `subjTextColor()` con variantes oscurecidas de los 6 colores de materia — todo pasa AA (4.5:1) como texto, mientras botones, barras, puntos y chips conservan el color brillante de identidad ("Azul cielo" intacto). **Contraste axe: 874 → 122 nodos (−86%).**
> - **Residuales conocidos (~122 nodos):** texto blanco sobre rellenos de color de marca (botones/badges — decisión de identidad, se conserva a 3:1 que es AA para texto grande), `--mute` sobre celdas con tinte, colores semánticos de dificultad/estado como texto, y la fecha de examen por defecto (editable en Configuración).
